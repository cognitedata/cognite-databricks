...
def register_udtf_from_file(
    ...
    with open(udtf_file_path, 'r', encoding='utf-8') as f:
        udtf_code = f.read()
    ...
    if not udtf_classes:
        raise ValueError(
            f"No UDTF class found in {udtf_file_path}. "
            "Expected a class with 'eval' and 'analyze' methods."
        )
    
    if len(udtf_classes) > 1:
        raise ValueError(
            f"Multiple UDTF classes found in {udtf_file_path}. Expected exactly one."
        )
    ...
    if not hasattr(udtf_class, 'analyze'):
        raise RuntimeError(
            f"analyze method not found in {udtf_class.__name__}! "
            "Required for PySpark Connect."
        )
...
def generate_time_series_udtf_view_sql(
    ...
    if config is None:
        raise ValueError(f"Unknown time series UDTF: {udtf_name}. Available UDTFs: {time_series_udtf_registry.get_all_udtf_names()}")
    
    # Use config values if not provided
    actual_view_name = view_name or config.view_name
    actual_params = udtf_params or config.parameters
...
def generate_udtf_sql_query(
    ...
    if use_named_parameters:
        ...
        sql_lines = [
            ...
            f"-- All view property parameters use default NULL values",
        ...
        sql_lines = [
            ...
            f"    -- View property parameters (all NULL to get all rows)",
        ]
...
def generate_session_scoped_notebook_code(
    ...
    # Try to get catalog and schema
    try:
        actual_catalog = generator.catalog
        actual_schema = generator.schema
    except Exception:
...
for view_id, func_name in registered.items():
    ...
    if view_property_names:
        positional_sql_lines = [
            f"-- Alternative: Positional parameters (if you need to specify some properties)",
            f"SELECT * FROM {example_function_name}(",
            f"    SECRET('{secret_scope}', 'client_id'),",
            f"    SECRET('{secret_scope}', 'client_secret'),",
            f"    SECRET('{secret_scope}', 'tenant_id'),",
        ...
        positional_sql_lines.append(f") LIMIT 10;")
...
class UDTFGenerator:
    ...
    def __init__(
        ...
        if code_generator is None:
            raise ValueError(
                "code_generator must be provided, or use generate_udtf_notebook() to create UDTFGenerator"
            )
        ...
        if rate_limiter:
            ...
            try:
                return self._register_single_udtf_and_view_impl(
                    view_id, udtf_file, view_sql, secret_scope, dependencies,
                    warehouse_id, if_exists, debug
                )
            finally:
    ...
    def register_udtfs_and_views(
        ...
        Args:
            ...
            dependencies: Optional Python package dependencies (DBR 18.1+).
                         ...
                         Example: ["cognite-sdk>=7.90.1", "cognite-pygen>=1.2.29"]  # Note: PyPI package name uses hyphens
            warehouse_id: Optional SQL warehouse ID for view registration.
        ...
        if debug:
            print(f"\n{'='*60}")
            print(f"[DEBUG] Starting UDTF and View registration")
    ...
    def _parse_udtf_params_from_class(
        ...
        if debug:
            print(f"[DEBUG] Secret parameters (5):")
        ...
        if debug:
            print(f"[DEBUG] UDTF-specific parameters:")
        ...
        for param_name, param in analyze_sig.parameters.items():
            ...
            if debug:
                print(f"  [{position}] {param_name}: type_text='{sql_type}', type_name={type_name}, default={default_value}")
    ...
    def _parse_return_params_from_class(
        ...
        for position, field in enumerate(struct_type.fields):
            ...
            if debug:
                print(f"  [{position}] {field.name}: type_text='{sql_type}', type_name={type_name}, nullable={field.nullable}")
    ...
    def _parse_udtf_params(self, view_id: str, debug: bool = False) -> list[FunctionParameterInfo]:
        ...
        for param_name, sql_type, spark_type_name in secret_params:
            ...
            type_json_value = TypeConverter.spark_to_type_json(spark_type, param_name, nullable=True)
            if debug:
                print(f"  [{position}] {param_name}: type_text='{sql_type}', type_name=STRING, type_json='{type_json_value}'")
            input_params.append(
                FunctionParameterInfo(
                    name=param_name,
                    type_text=sql_type,
        ...
        for prop_name, prop in view.properties.items():
            ...
            if is_relationship:
                if is_multi:
                    ...
                    if debug:
                        print(f"  [SKIP] {prop_name}: MultiReverseDirectRelation - skipping (ARRAY type_json format not supported by API yet)")
                    continue
                else:
                    ...
                    if debug:
                        print(f"  [{position}] {prop_name}: type_text='STRING', type_name=ColumnTypeName.STRING (single relationship - external_id reference)")
            else:
                ...
                type_json_value = TypeConverter.spark_to_type_json(spark_type, prop_name, nullable=True)
                if debug:
                    print(f"  [{position}] {prop_name}: type_text='{sql_type}', type_name={type_name}, type_json='{type_json_value}'")
    ...
    def validate_schema_consistency(
        self, 
        view_id: str, 
        registered_function: FunctionInfo
    ) -> tuple[bool, str | None]:
    ...
    def _schemas_match(self, schema1: StructType, schema2: StructType) -> tuple[bool, str | None]:
        ...
        for i, (f1, f2) in enumerate(zip(schema1.fields, schema2.fields)):
            if f1.name != f2.name:
    ...
    def _extract_and_register_foreign_keys(
        ...
        for prop_name, prop in view.properties.items():
            ...
            if is_multi:
                if debug:
                    print(f"[DEBUG] Skipping FK constraint for {view_id}.{prop_name} (MultiReverseDirectRelation - arrays not supported in FK constraints)")
                continue
            ...
            if referenced_view_id:
                ...
                print(f"[DEBUG] Relationship property {view_id}.{prop_name} found but could not determine referenced view")
